...
//  aizen
//
//  PNPM package installation for ACP agents
//
...
actor PNPMInstaller {
    static let shared = PNPMInstaller()

    private let shellLoader: ShellEnvironmentLoader
    private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "com.aizen.app", category: "PNPMInstaller")
    ...
    func install(package: String, targetDir: String) async throws {
        ...
        process.arguments = ["pnpm", "install", "--prefix", targetDir, package]

        // Load shell environment to get PATH with pnpm
