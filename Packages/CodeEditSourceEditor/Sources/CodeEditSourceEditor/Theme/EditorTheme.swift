...
public struct EditorTheme: Equatable {
    ...
    private func mapCapture(_ capture: CaptureName?) -> Attribute {
        ...
        case .textTitle, .textStrong:
            return Attribute(color: keywords.color, bold: true, italic: false) // Headings and bold - prominent
        case .textEmphasis:
            return Attribute(color: comments.color, bold: false, italic: true) // Italic text
        case .textUri, .textReference:
            return attributes // Links and references
        case .textLiteral:
            return strings // Code blocks and inline code
        case .punctuationSpecial, .punctuationDelimiter:
            return attributes // Markdown syntax markers
        case .stringEscape:
            return attributes // Escape sequences
        // Swift and other language captures
        case .functionCall:
            return commands // Function calls like print(), map()
        case .functionMacro:
            return attributes // Preprocessor macros/directives like #if
        case .keywordOperator, .operator:
            return attributes // Operators like +, -, &&, ||
        case .label:
